...
- Now contract is published to npm as `@1inch/limit-order-protocol`
...
- Now contract is published to npm as `@1inch/limit-order-protocol-utils`
...
- `LimitOrderProtocolFacade.fillLimitOrder` and `fillOrderToWithPermit` have more abilities now:
    ...
    - `skipPermit` wether to skip maker's permit evaluation if it was evaluated before.
        Useful if multiple orders were created with same nonce.
...
- `simulate(address, calldata): Promise<{ success, rawResult }>` was introduced instead, so you don't need `catch` block anymore.
