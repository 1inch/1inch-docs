...
* **Pool governance** only allows for the configuration of parameters specific to each pool. These parameters include the swap fee, the price impact fee, and the decay period. Liquidity providers may participate in pool governance by staking that pool's LP tokens -- the weight of each user’s vote is proportional to the amount of LP tokens they have staked. The Liquidity Protocol uses a weighted average of all votes and applies it linearly over 24 hours.
